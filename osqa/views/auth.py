...
from osqa.authentication.forms import SimpleRegistrationForm, SimpleEmailSubscribeForm, \
        TemporaryLoginRequestForm, ChangePasswordForm, SetPasswordForm
from osqa.utils.mail import send_email

from osqa.authentication.base import InvalidAuthentication
from osqa.authentication import AUTH_PROVIDERS, user_logged_in

from osqa.models import AuthKeyUserAssociation, ValidationHash, Question, Answer
...
def external_register(request):
    ...
    return render_to_response('osqa/auth/complete.html', {
        'form1': form1,
...
def request_temp_login(request):
    ...
    return render_to_response(
            'osqa/auth/temp_login_request.html', {'form': form}, 
            context_instance=RequestContext(request))
...
def auth_settings(request):
    ...
    return render_to_response('osqa/auth/auth_settings.html', {
        'form': form,
